...
Function.prototype.derive = function(constructor, proto){
    ...
    var parent = this; // a
    var fn = function(){ // 定义函数fn，执行时会执行 a 及 constructor 构造器
...
//require
//加载插件模块
//如：fis.require('command', 'release')
fis.require = function(){
    var path;
    var name = Array.prototype.slice.call(arguments, 0).join('-'); // 'command-release'
    if(fis.require._cache.hasOwnProperty(name)) return fis.require._cache[name]; // 如果缓存中存在当前插件模块，则返回。
    var names = [];
...
//package info
//读取当前目录下的的配置信息
//参考 https://nodejs.org/api/globals.html#globals_dirname
