...
var fs = require('fs'),
    ...
    Url = require('url'),
    //判断文件是否存在，fs.exists()的同步版本，未来会被废弃。
    //参考 https://nodejs.org/api/fs.html#fs_fs_existssync_path
    _exists = fs.existsSync || pth.existsSync,
    toString = Object.prototype.toString,
    iconv, tar;  //iconv 存储加载过的'iconv-lite'模块；tar 存储加载过的'tar'模块。

//判断是否是windows平台
var IS_WIN = process.platform.indexOf('win') === 0; //'win32'
...
var TEXT_FILE_EXTS = [
    ...
    IMAGE_FILE_EXTS = [ //image文件后缀列表（文件类型）
        'svg', 'tif', 'tiff', 'wbmp',
        'png', 'bmp', 'fax', 'gif',
        'ico', 'jfif', 'jpe', 'jpeg',
        'jpg', 'woff', 'cur', 'webp',
...
//加载'iconv-lite'模块（编码模块），返回接口iconv。
function getIconv(){
    if(!iconv){
...
var _ = module.exports = function(path){
    ...
    if(arguments.length > 1) {
        path = Array.prototype.join.call(arguments, '/'); //将参数中传递的路径用'/'联合起来
    } else if(type === 'string') {
        //do nothing for quickly determining.
    } else if(type === 'object') {
        path = Array.prototype.join.call(path, '/'); //将数组中的路径字符串用'/'联合起来
    } else if(type === 'undefined') {
        path = '';
    }
    if(path){
        //参考 https://nodejs.org/api/path.html#path_path_normalize_p
        //如：'/foo/bar//baz/asdf/quux/..' => ''/foo/bar/baz/asdf'
        path = pth.normalize(path.replace(/[\/\\]+/g, '/')).replace(/\\/g, '/');
        if(path !== '/'){
            path = path.replace(/\/$/, ''); //如：'a/b/c/' => 'a/b/c'
        }
    }
    return path;
};

//判断数据类型，type如：'Array'、'Object'等。
_.is = function(source, type){
    return toString.call(source) === '[object ' + type + ']';
};

//实现了ECMAScript5中数组方法map。同时增加了新特性merge，向回调添加静态属性。
_.map = function(obj, callback, merge){
    var index = 0;
    for(var key in obj){
        if(obj.hasOwnProperty(key)){
            if(merge){ //如果merge是true，则callback可以是对象，可以是回调函数。
...
_.clone = function(source) {
    ...
    switch(toString.call(source)){
        case '[object Object]': //克隆对象
            ret = {};
            _.map(source, function(k, v){
                ret[k] = _.clone(v);
            });
            break;
        case '[object Array]': //克隆数组
            ret = [];
...
//从一串字符串中获取相关信息，如：
//'abcdefg ' => {origin: "abcdefg ", rest: "abcdefg", quote: "'"}
...
_.getMimeType = function(ext){
    ...
    return MIME_MAP[ext] || 'application/x-' + ext; //如：'css' => 'text/css'
};
...
//参考 https://nodejs.org/api/fs.html#fs_fs_realpathsync_path_cache
_.realpath = function(path){
    if(path && _exists(path)){
        path = fs.realpathSync(path);
        //如果是windows操作系统平台，替换路径中的'\\'为'/'。
        if(IS_WIN){
            path = path.replace(/\\/g, '/');
        }
        //去掉最后一个'/'，如：'a/b/c/' => 'a/b/c'
        if(path !== '/'){
            path = path.replace(/\/$/, '');
        }
        return path;
...
_.isAbsolute = function(path) {
    ...
    if (IS_WIN) {
        ...
        if(path === '/'){ //如：'/'
            ...
            if(split[0] === '~'){
                return true;
            } else if(split[0] === '' && split[1]) { //如：'/data/www/tedyhy'
...
//判断是否是文件。参考 https://nodejs.org/api/fs.html#fs_fs_statsync_path
//fs.statSync获取文件详细信息。参考 https://nodejs.org/api/fs.html#fs_class_fs_stats
...
_.touch = function(path, mtime){
    ...
    if(mtime instanceof Date){
        //do nothing for quickly determining.
    } else if(typeof mtime === 'number') { //mtime是时间戳
        var time = new Date();
        time.setTime(mtime);
        mtime = time;
    } else {
...
_.write = function(path, data, charset, append){
    //判断文件是否存在，不存在则创建目录
    if(!_exists(path)){
        _.mkdir(_.pathinfo(path).dirname); //创建目录
    }
    //设置编码
    if(charset){
...
_.ext = function(str){
    ...
    //获取文件后缀
    if((pos = str.lastIndexOf('.')) > -1){
        info.ext = str.substring(pos).toLowerCase(); //文件后缀
        info.filename = str.substring(0, pos); //文件名
        info.basename = info.filename + info.ext; //包含后缀的文件名
    } else {
...
//获取文件相关信息，如：hash、query。
_.query = function(str){
    var rest = str,
        pos = rest.indexOf('#'),
        hash = '',
        query = '';
    if(pos > -1){ //如：'/a/b/c?b=1#a'
        hash = rest.substring(pos); //'#a'
        rest  = rest.substring(0, pos); //'/a/b/c?b=1'
    }
    pos = rest.indexOf('?');
    if(pos > -1){
        query = rest.substring(pos); //'?b=1'
        rest  = rest.substring(0, pos); //'/a/b/c'
    }
    rest = rest.replace(/\\/g, '/'); //如：'\a\b\c' => '/a/b/c'
    if(rest !== '/'){
        rest = rest.replace(/\/\.?$/, ''); //如：/'a/b/./c' => '/a/b/c'
    }
    //返回url分析后的信息
    return {
        origin : str, //原始url
        rest : rest, //去除hash、query后的url
        hash : hash, //hash内容
        query : query //query内容
    };
};
...
_.pathinfo = function(path){
    ...
    var type = typeof path;
    if(arguments.length > 1) { //如：_.pathinfo('a', 'b', 'c') => 'a/b/c'
        path = Array.prototype.join.call(arguments, '/');
    } else if(type === 'string') {
        //do nothing for quickly determining.
    } else if(type === 'object') { //如：['a', 'b', 'c'] => 'a/b/c'
        path = Array.prototype.join.call(path, '/');
    }
    return _.ext(path);
};

//驼峰式书写，如：'abc-def_g igk' => 'abcDefGIgk'。
